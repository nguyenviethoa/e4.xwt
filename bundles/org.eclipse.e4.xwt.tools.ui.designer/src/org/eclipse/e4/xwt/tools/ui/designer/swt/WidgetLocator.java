...
/**
 * Provides a means to find the coordinates of an SWT Widget in display-space, given an SWT Display object.
 * 
 * @author jliu (jin.liu@soyatec.com)
 */
public class WidgetLocator {
	/**
	 * Helper method to convert a Rectangle, given in the coordinate system of the given Control's parent, to display-cordinates.
	 */
	...
	public static Rectangle getBounds(Widget w) {
		if (w instanceof Control) {
			if (w instanceof Decorations) {
				...
				Rectangle calced = ((Decorations) w).computeTrim(bounds.x, bounds.y, clientArea.width, clientArea.height);
				Rectangle correct = new Rectangle(2 * bounds.x - calced.x, 2 * bounds.y - calced.y, clientArea.width, // bug workaround
						clientArea.height);
		...
		if (w instanceof Menu) {
			...
			} else if (StyleHelper.checkStyle(menu, SWT.BAR)) {
	...
	private static Rectangle getBounds(MenuItem menuItem) {
		...
		if ((SWT.SEPARATOR & menuItem.getStyle()) != 0) {
			...
			Dimension textExtents = FigureUtilities.getTextExtents(text, Display.getCurrent().getSystemFont());
