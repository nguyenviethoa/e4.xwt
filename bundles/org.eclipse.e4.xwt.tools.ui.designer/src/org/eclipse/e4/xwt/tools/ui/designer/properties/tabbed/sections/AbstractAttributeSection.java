...
public abstract class AbstractAttributeSection extends AbstractPropertySection implements Listener {
	...
	protected void performRefresh(Notification msg) {
		...
		if (notifier == attribute) {
			refresh();
		} else if (notifier == parent && (attribute == msg.getOldValue() || attribute == msg.getNewValue())) {
			refresh();
		}
	}
	...
	public XamlAttribute getAttribute() {
		...
		if (attribute != null && attribute.eContainer() == null) {
			attribute = XWTModelUtil.getAdaptableAttribute(parent, getAttributeName(), IConstants.XWT_NAMESPACE);
		}
		if (attribute == null) {
	...
	protected void applyEditPart(WidgetEditPart editPart) {
		...
		if (attrLable != null && !attrLable.isDisposed()) {
			...
			if (name != null && name.length() > 0) {
				attrLable.setText(Character.toUpperCase(name.charAt(0)) + name.substring(1) + ":");
	...
	protected RefreshAdapter getRefresher() {
		if (refreshAdapter == null) {
			refreshAdapter = new RefreshAdapter(getParent()) {
				protected void performRefresh(Notification msg) {
					refresh();
				}
			};
	...
	public void handleEvent(Event event) {
		...
		if (parent.getAttributes().contains(attribute)) {
			command = new ApplyAttributeSettingCommand(parent, attribute, getNewValue(event));
		} else {
						...
						if (value != null) {
							...
							if (!type.isAssignableFrom(value.getClass()) || value instanceof IBinding) {
								...
								if (converter != null) {
									...
									if (value != null && orginalValue instanceof IBinding && !type.isAssignableFrom(value.getClass())) {
										converter = ((XWTVisualLoader) xwtLoader).findConvertor(value.getClass(), type);
										if (converter != null) {
											value = converter.convert(value);
										} else {
											LoggerManager.log(new XWTException("Convertor " + value.getClass().getSimpleName() + "->" + type.getSimpleName() + " is not found"));
										}
									}
								}
							}
	...
	protected Object getStaticValue(XamlNode child, IXWTLoader xwtLoader) {
		...
		if (children.length == 1) {
			...
			if (element != null) {
				return ClassLoaderUtil.loadStaticMember(((XWTVisualLoader) xwtLoader).getLoadingContext(), element);
			}
		}
		return null;
	}
