...
public class DesignerModelBuilder extends AdapterImpl implements IModelBuilder {
	...
	protected XamlElement createElement(XamlNode parent, IDOMElement child, int index) {
		...
		if (e == null || e.getName() == null || e.getNamespace() == null || !e.getName().equals(name) || !e.getNamespace().equals(ns)) {
	...
	protected XamlAttribute getAttribute(XamlNode parent, String attrName, String namespace) {
		...
		return parent.getAttribute(attrName, namespace);
	}
			...
			for (int i = 0; i < childNodes.getLength(); i++) {
				...
				if (item.getNodeType() == Node.TEXT_NODE) {
					...
					if (t.isWhitespace() || t.isInvalid()) {
						...
						while (next != null && next.getNodeType() == Node.TEXT_NODE && (((TextImpl) next).isWhitespace() || ((TextImpl) next).isInvalid())) {
	...
	/**
	 * @see org.soyatec.IModelBuilder.ve.editor.model.IDiagramModelBuilder#addModelBuildListener(org.soyatec.ModelBuildListener.ve.editor.model.IModelChangeListener)
	 */
	...
	/**
	 * @see org.soyatec.IModelBuilder.ve.editor.model.IDiagramModelBuilder#removeModelBuildListener(org.soyatec.ModelBuildListener.ve.editor.model.IModelChangeListener)
	 */
